...
类似于kylin的ExecMan, [ExecutionQueue](https://github.com/brpc/brpc/blob/master/src/bthread/execution_queue.h)提供了异步串行执行的功能。ExecutionQueue的相关技术最早使用在RPC中实现[多线程向同一个fd写数据](../../rpc-in-depth/io#sending-messages). 在r31345之后加入到bthread。 ExecutionQueue 提供了如下基本功能:
...
- 如果临界区非常小，竞争又不是很激烈，优先选择使用mutex, 之后可以结合[contention profiler](../../builtin-services/contention_profiler/)来判断mutex是否成为瓶颈。
