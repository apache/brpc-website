...
- 第一类协议：标记或特殊字符在最前面，比如[baidu_std](https://github.com/apache/incubator-brpc/blob/master/docs/cn/baidu_std.md)，hulu_pbrpc的前4个字符分别是PRPC和HULU，解析代码只需要检查前4个字节就可以知道协议是否匹配，最先尝试这类协议。这些协议在同一个连接上也可以共存。
...
> 以nshead开头的协议有统一支持，看[这里](https://github.com/apache/incubator-brpc/blob/master/docs/cn/nshead_service.md)。
