...
User can extend to more naming services by implementing brpc::NamingService, check [this link](../../rpc-in-depth/load-balancing/#命名服务) for details.
...
You can new these objects individually and create done by [NewCallback](#use-newcallback), or make response/controller be member of done and [new them together](#inherit-googleprotobufclosure). Former one is recommended.
...
NOTE: [ParallelChannel](../combo-channels/#parallelchannel) is probably more convenient to  launch multiple RPCs in parallel.
...
- PROTOCOL_UBRPC_COMPACK or "ubrpc_compack", which is protocol of public/ubrpc, packing with compack, using pooled connection by default. check out [ubrpc (by protobuf)](../access-ub/) for details. A related protocol is PROTOCOL_UBRPC_MCPACK2 or ubrpc_mcpack2, packing with mcpack2.
- PROTOCOL_NSHEAD_CLIENT or "nshead_client", which is required by UBXXXRequest in baidu-rpc-ub, using pooled connection by default. Check out [Access UB](../access-ub/) for details.
- PROTOCOL_NSHEAD or "nshead", which is required by sending NsheadMessage, using pooled connection by default. Check out [nshead+blob](../access-ub/#nshead-blob) for details.
...
brpc also supports [Streaming RPC](../streaming-rpc/) which is an application-level connection for transferring streaming data.
...
Check lifetime of Controller, Response and done. In asynchronous call, finish of CallMethod is not completion of RPC which is entering of done->Run(). So the objects should not deleted just after CallMethod, instead they should be delete in done->Run(). Generally you should allocate the objects on heap instead of putting them on stack. Check out [Asynchronous call](#asynchronous-call) for details.
